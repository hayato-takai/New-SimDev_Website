...
// Typing a fluid property by hand detaches it from the selected oil, so the
// selection is cleared rather than left claiming a value it no longer supplies.
// updateOilProps() writes these fields programmatically, which does not fire
// "input", so refilling them from an oil cannot clear the selection it just set.
function detachFromOil() {
  const fluidSelect = document.getElementById("oilTypeSelect");
  if (fluidSelect.value !== "") fluidSelect.value = "";
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("viscosityInput").addEventListener("input", detachFromOil);
  document.getElementById("densityInput").addEventListener("input", detachFromOil);
});
